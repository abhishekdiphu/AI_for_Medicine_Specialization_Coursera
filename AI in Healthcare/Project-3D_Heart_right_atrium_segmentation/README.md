# Sementic Segmentation of 3D volume of Heart's right atrium.
...
We are using the "heart" dataset from the [Medical Decathlon competition](http://medicaldecathlon.com/). This dataset is stored as a collection of NIFTI files, with one file per volume, and one file per corresponding segmentation mask. The original images here are T2 MRI scans of the full brain. As noted, in this dataset we are using cropped volumes where only the region around the heart has been cut out. This makes the size of our dataset quite a bit smaller, our machine learning problem a bit simpler and allows us to have reasonable training times. We should not think of it as "toy" problem, though. Algorithms that crop rectangular regions of interest are quite common in medical imaging.
...
<img src="./Exploratory_analysis/img/hist0gram.png" width=400px>

###### Sagittal plane:

<img src="./Exploratory_analysis/img/sagittal_plane_mip.png" width=200px>

###### Corornal and axial plane :

<img src="./Exploratory_analysis/img/coronal_axial.png" width=200px>
...
### Notes:
- the volume (in mm³) of a right atrium: 96672.
- Spatial unit: mm, Temporal unit: sec
- Axis are : , Coronal: 0, Axial: 1,  Sagittal: 2
- pixdim are : 1.25 1.25 1.37


#### Image Preprocessing :
- volumes are approximately of size 320 x 320 x 100.
- Since the unet is a 2d architecure , so , slices are made of single channels for the architecture.
- Once these slices are created , they are cropped in a  shape of 128 x 128 x 40 , with padding on the sides , keeping the aspect ratio of the original dimension intact.
